...
async function runSync(onProgress) {
  ...
  try {
    ...
    const headRes = await fetch(
      `${config.github.apiBase}/repos/${config.github.owner}/${config.github.repo}/git/ref/heads/${encodeURIComponent(config.github.branch)}`,
      { headers: ghHeaders() },
    );
    ...
    // 2. 下载分支 tarball（codeload，单请求包含全仓库；经 GITHUB_WEB_BASE 可走代理）
    const tarRes = await fetch(
      `${config.github.webBase}/${config.github.owner}/${config.github.repo}/tar.gz/refs/heads/${encodeURIComponent(config.github.branch)}`,
      { headers: ghHeaders() },
    );
    ...
    report({ sha });
    await pipeline(tarRes.body, countBytes(tarRes, (n) => report({ bytes: n })), createWriteStream(tarFile));

    // 3. 解压进 staging，并把唯一顶层目录（codeload 为 <owner>-<repo>-<ref>）的内容上移一层
    report({ bytes: (await fs.stat(tarFile)).size });
    ...
    report({ status: "ok", finishedAt: Date.now(), files });
  } catch (err) {
    report({ status: "error", error: err.message || "同步失败", finishedAt: Date.now() });
  }
  return syncState();
}
