...
async function runDeploy(rec) {
  ...
  git(dir, ["fetch", "origin", branch, "--depth=1"]);
...
/** 在部署目录执行 git 命令，出错抛出带 stderr 的 Error。 */
function git(dir, args) {
  try {
    // -c safe.directory：仓库由其他用户（如 root）克隆时 git 会拒绝操作，
    // 显式声明信任，不依赖运行用户的 global config（www-data 常无 HOME 可写）
