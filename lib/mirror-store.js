...
/** git blob sha（sha1("blob <size>\0" + 原始字节)），与 GitHub Contents API
 *  返回的 sha 同源。镜像文件由 codeload tarball 解压而来，字节与仓库 blob
 *  完全一致，直接现算即可，无需 sidecar 索引 */
function blobShaOf(buf) {
  return crypto.createHash("sha1").update(Buffer.from(`blob ${buf.length}\0`)).update(buf).digest("hex");
}

/** 读取文件：返回 { contentRaw, sha, path }；不存在时返回 null
 *  sha 用原始字节按 git blob 算法现算（tarball 内为仓库原始字节，含 LF 换行）——
 *  GitHub 更新/删除接口要求真实 blob sha，null 会被 422 拒绝 */
export async function getFile(relPath) {
  try {
    const buf = await fs.readFile(resolveLocal(relPath));
    return { contentRaw: buf.toString("utf-8"), sha: blobShaOf(buf), path: relPath };
  } catch (err) {
...
/** 读取二进制文件：返回原始 Buffer；不存在时返回 null（图片预览/管理用） */
export async function getBinaryFile(relPath) {
  try {
    return await fs.readFile(resolveLocal(relPath));
  } catch (err) {
    if (err.code === "ENOENT") return null;
    throw err;
  }
...
/** 列出整棵树（tarball 仅含 content/），返回 [{ path, type }] */
export async function listTree() {
  ...
  // tarball 经 export-ignore 只含 content/；public/ 图片直推仓库、读取走线上站点，镜像不缓存
...
/** 推进镜像 meta 到指定 commit（写后回写用：自身提交不让 remoteAhead 误报） */
export async function advanceMeta(sha) {
  if (!sha) return;
  const meta = lastMeta();
  if (!meta) return;
  meta.sha = sha;
  meta.syncedAt = new Date().toISOString();
  await fs.writeFile(path.join(currentPath(), "meta.json"), JSON.stringify(meta, null, 2), "utf-8").catch(() => {});
}
