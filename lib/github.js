...
/**
 * 读取二进制文件（图片等）：返回 Buffer；不存在时返回 null。
 * 用 raw Accept 直接取原始字节——contents JSON 的 base64 仅支持 ≤1MB，
 * 且 JSON 响应经 utf-8 字符串化会破坏二进制内容。
 */
export async function getBinaryFile(path) {
  const res = await fetch(
    `${API_BASE}/repos/${repo()}/contents/${encodePath(path)}?ref=${encodeURIComponent(config.github.branch)}`,
