...
/** 读取二进制文件（图片预览/管理）：镜像优先，未命中回退 GitHub raw */
export async function getBinaryFile(path) {
  if (isDev()) return devStore.getBinaryFile(path);
  if (mirror.isReady()) {
    const buf = await mirror.getBinaryFile(path);
    if (buf) return buf;
  }
  return github.getBinaryFile(path);
}
